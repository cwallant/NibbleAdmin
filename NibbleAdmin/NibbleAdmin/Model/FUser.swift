...
func saveUserToFirestore(fUser: FUser) {
    ...
    FirebaseReference(collectionReference: .User).document(fUser.id).setData(userDictionaryFrom(user: fUser)) { (error) in
...
func updateCurrentUser(withValues: [String : Any], completion: @escaping (_ error: Error?) -> Void) {
    ...
    if let dictionary = userDefaults.object(forKey: kCURRENTUSER) {
        ...
        FirebaseReference(collectionReference: .User).document(FUser.currentId()).updateData(withValues) { (error) in
